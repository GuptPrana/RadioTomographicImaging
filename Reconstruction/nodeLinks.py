...
def generateGrid(delta=6.25,nodeFile='defineNodes.json',corners=0,ROI='e'):
    ...
    if (ROI=='b'):
        ...
        # minX, minY = 0, 0
        maxX = int((maxX-minX)/delta)+((maxX-minX)%delta>0)
        maxY = int((maxY-minY)/delta)+((maxY-minY)%delta>0)
        return maxX, maxY
...
def generateAllWeightsBres(nodeFile='defineNodes.json', linksFile='defineLinks.json', weightsFile='defineWeightsEll.json', delta=6.25):
    # Grid Template
...
def generateWeights(pointA, pointB, maxX, maxY):
    ...
    distX = mesh[0]
    distY = mesh[1]
    distA = np.sqrt((distX - x1)**2 + (distY - y1)**2)
    distB = np.sqrt((distX - x2)**2 + (distY - y2)**2)
    
    grid = (distA+distB)<=dist
    return grid


def generateAllWeights(nodeFile='defineNodes.json', linksFile='defineLinks.json', weightsFile='defineWeightsEll.json', delta=6.25):
    # Grid Template
    # grid, minX, maxX, minY, maxY = generateGrid(corners=1, ROI='b')
    maxX, maxY = generateGrid()
    nodes = json.load(open(nodeFile))
...
# Lambda needs to be tuned 0 < lbd < 1
